...
PREVIOUS_DIR=$(pwd)
cd $HOME/samples/operator/MatMulCustomSample/KernelLaunch/MatMulInvocation
# clean temp files
...
# 检查精度是否正常
echo "####### start run.sh #######"
bash ./run.sh npu > $TEMP_DIR/temp_run.txt
if [ $? -eq 0 ];then
...
# extract .o
llvm-objcopy -j .aicore_binary -O binary ./matmul_custom_npu $TEMP_DIR/temp_kernel.o
