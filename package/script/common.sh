...
MODULE_NAME="mindstudio-debugger"
LEVEL_ERROR="ERROR"
LEVEL_WARN="WARNING"
LEVEL_INFO="INFO"
USERNAME=$(id -un)
...
MINDSTUDIO_ARRAY=("mindstudio-debugger")
...
function remove_uninstall_package() {
    ...
    if [ -f "${_file}" ]; then
        sed -i "/uninstall_package \"share\/info\mindstudio-debugger\/script\"/d" "${_file}"
        if [ $? -ne 0 ]; then
...
function uninstall_tool() {
    ...
    "$COMMON_PARSER_PATH" --restoremod --package=mindstudio-sanitizer --username="unknown" --usergroup="unknown" \
    ...
    "$COMMON_PARSER_PATH" --remove --package=mindstudio-sanitizer "${install_path}" "${FILELIST_CSV_PATH}"
    if [ $? -ne 0 ]; then
        log_and_print $LEVEL_ERROR "ERR_NO:0X0090;ERR_DES: Remove mindstudio-sanitizer files failed in ${install_path}."
        return 1
    fi
