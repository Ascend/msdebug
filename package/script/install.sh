...
function install_tool() {
    "$COMMON_PARSER_PATH" --makedir --package=mindstudio-debugger --username="${USERNAME}" --usergroup="${USERGROUP}" \
        "${install_path}" "${FILELIST_CSV_PATH}"
    if [ $? -ne 0 ]; then
        log_and_print $LEVEL_ERROR "Create mindstudio-debugger subfolders failed."
        return 1
    fi

    "$COMMON_PARSER_PATH" --copy --package=mindstudio-debugger --username="${USERNAME}" --usergroup="${USERGROUP}" \
        --install_for_all=${install_for_all} "${install_path}" "${FILELIST_CSV_PATH}"
    if [ $? -ne 0 ]; then
        log_and_print $LEVEL_ERROR "Copy mindstudio-debugger files failed."
        return 1
    fi

    "$COMMON_PARSER_PATH" --chmoddir --package=mindstudio-debugger --username="${USERNAME}" --usergroup="${USERGROUP}" \
        --install_for_all=${install_for_all} "${install_path}" "${FILELIST_CSV_PATH}"
    if [ $? -ne 0 ]; then
        log_and_print $LEVEL_ERROR "Chmod mindstudio-debugger files failed."
        return 1
    fi

    if [ ! -f ${install_path}"/cann_uninstall.sh" ]; then
        cp ${install_path}/share/info/mindstudio-debugger/script/cann_uninstall.sh ${install_path}
    fi
    chmod -Rf 500 "${install_path}/share/info/mindstudio-debugger/script"
    return 0
}
...
function install_pkg() {
    install_file=${install_path}/share/info/mindstudio-debugger/ascend_install.info
    uninstall_file=${install_path}/share/info/mindstudio-debugger/script/uninstall.sh
    if [ $(id -u) -eq 0 ]; then
            install_for_all=y
    fi
...
function execute_with_lock() {
    local lock_path="$1"
    local lockfile="${lock_path}/.mindstudio-debugger.lock"
    touch "$lockfile"
